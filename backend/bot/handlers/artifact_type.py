...
async def change_artifact_type(message: types.Message):
    try:
        await message.answer(text=localization_enum.Messages.ARTIFACT_CHOOSE_TYPE.value,
...
async def set_artifact_type(call: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await call.answer(
            localization_enum.Messages.ARTIFACT_CHOOSE_TYPE_SUCCESS.value.format(call.data),
        )
    except Exception as e:
        logger.error(e)
        await call.answer(
